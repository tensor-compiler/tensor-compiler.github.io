...
// ./build/bin/taco "y(i)=A(i,j)*x(j)" -f=y:d:0 -f=A:ds:0,1 -f=x:d:0 -write-source=taco_kernel.c
...
int main() {
  ...
  int32_t* A_dimensions = malloc(sizeof(int32_t) * (A_order + 1));
  A_dimensions[0] = 5;
  A_dimensions[1] = 6;

  int32_t* A_modeOrdering = malloc(sizeof(int32_t) * (A_order + 1));
  A_modeOrdering[0] = 0;
  A_modeOrdering[1] = 1;

  taco_mode_t* A_mode_types = malloc(sizeof(taco_mode_t) * (A_order + 1)); 
  A_mode_types[0] = taco_mode_dense; 
  A_mode_types[1] = taco_mode_sparse; 

  // initialize vectors x, y 
  int32_t vec_order = 1; 
  int32_t vec_csize = 42; 

  int32_t* x_dimensions = malloc(sizeof(int32_t) * (vec_order + 1));
  x_dimensions[0] = A_dimensions[1]; 

  int32_t* y_dimensions = malloc(sizeof(int32_t) * (vec_order + 1));
  y_dimensions[0] = A_dimensions[0]; 

  int32_t* vec_modeOrdering = malloc(sizeof(int32_t) * (vec_order + 1));
  vec_modeOrdering[0] = 0;

  taco_mode_t* vec_mode_types = malloc(sizeof(taco_mode_t) * (vec_order + 1)); 
  vec_mode_types[0] = taco_mode_dense; 

  // set up input A in COO format
  int A_nnz = 3; 
  int* A_pos1 = malloc(sizeof(int) * (2 + 1));
  int* A_crd1 = malloc(sizeof(int) * (A_nnz + 1)); 
  int* A_crd2 = malloc(sizeof(int) * (A_nnz + 1)); 
  double* A_vals = malloc(sizeof(double) * (A_nnz + 1));

  A_pos1[0] = 0; A_pos1[1] = A_nnz;
  A_crd1[0] = 0; A_crd2[0] = 0; A_vals[0] = 1;
  A_crd1[1] = 3; A_crd2[1] = 4; A_vals[1] = 2;
  A_crd1[2] = 4; A_crd2[2] = 3; A_vals[2] = 3; 

  // set up input x in COO format
  int x_nnz = 4;
  int* x_pos1 = malloc(sizeof(int) * (2 + 1));
  int* x_crd1 = malloc(sizeof(int) * (x_nnz + 1));
  double* x_vals = malloc(sizeof(double) * (x_nnz + 1));

  x_pos1[0] = 0; x_pos1[1] = x_nnz;
  x_crd1[0] = 0; x_vals[0] = 1;
  x_crd1[1] = 2; x_vals[1] = 2;
  x_crd1[2] = 4; x_vals[2] = 3;
  x_crd1[3] = 5; x_vals[3] = 4; 

  // set up arrays for output y in COO format
  int* y_pos1;
  int* y_crd1;
  double* y_vals;

  // perform computation
  taco_tensor_t* A = init_taco_tensor_t(A_order, A_csize, A_dimensions, 
                                        A_modeOrdering, A_mode_types);
  taco_tensor_t* x = init_taco_tensor_t(vec_order, vec_csize, x_dimensions,
                                        vec_modeOrdering, vec_mode_types);
  taco_tensor_t* y = init_taco_tensor_t(vec_order, vec_csize, y_dimensions,
                                        vec_modeOrdering, vec_mode_types);

  pack_A(A, A_pos1, A_crd1, A_crd2, A_vals);
  pack_x(x, x_pos1, x_crd1, x_vals);

  evaluate(y, A, x);

  unpack(&y_pos1, &y_crd1, &y_vals, y);
  ...
  int output_nnz = y_dimensions[0];
  assert(y_pos1[1] == output_nnz); 

  for (int i = 0; i < output_nnz; i++) {
    assert(y_crd1[i] == i);
